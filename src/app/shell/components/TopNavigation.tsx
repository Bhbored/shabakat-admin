...
const ROUTE_TITLES: Record<string, { title: string; subtitle: string }> = {
  "/dashboard": { title: "Dashboard", subtitle: "Platform-wide operations snapshot" },
  "/companies": { title: "Companies", subtitle: "Manage registered companies and their details" },
...
export function TopNavigation() {
  ...
  const current = ROUTE_TITLES[location.pathname] ?? ROUTE_TITLES["/dashboard"];
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-border bg-background/85 px-4 backdrop-blur-xl md:px-6">
      <button onClick={toggleSidebar} className="rounded-lg p-1 text-muted-foreground transition-colors hover:text-foreground md:hidden">
      ...
      <div className="ms-auto flex items-center gap-2">
        ...
        <Avatar name={session?.fullName ?? "Admin"} />
      </div>
    </header>
  );
}
